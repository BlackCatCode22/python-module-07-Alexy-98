...
with open(f'zooPopulation_{current_date.strftime("%Y-%m-%d_%H%M")}.txt', 'w') as report_file:
    ...
    for hyena in list_of_hyenas:
        report_file.write(
            ...
            + hyena.sex + '; ' + hyena.weight + '; Sound: ' + hyena.hsound + '; From: ' + hyena.origin + '; Arrival Date: ' + str(
                hyena.date_arival.date()) + '\n')
    report_file.write('\nLion Habitat:\n')
    for lion in list_of_lions:
        report_file.write(
            lion.animal_id + ', ' + lion.name + '; Birthdate: ' + str(lion.birth_date) + '; ' + lion.color + '; '
            + lion.sex + '; ' + lion.weight + '; Sound: ' + lion.lsound + '; From: ' + lion.origin + '; Arrival Date: ' + str(
                lion.date_arival.date()) + '\n')
    report_file.write('\nTiger Habitat:\n')
    for tiger in list_of_tigers:
        report_file.write(
            tiger.animal_id + ', ' + tiger.name + '; Birthdate: ' + str(tiger.birth_date) + '; ' + tiger.color + '; '
            + tiger.sex + '; ' + tiger.weight + '; Sound: ' + tiger.tsound + '; From: ' + tiger.origin + '; Arrival Date: ' + str(
                tiger.date_arival.date()) + '\n')
    report_file.write('\nBear Habitat:\n')
    for bear in list_of_bears:
        report_file.write(
            bear.animal_id + ', ' + bear.name + '; Birthdate: ' + str(bear.birth_date) + '; ' + bear.color + '; '
            + bear.sex + '; ' + bear.weight + '; Sound: ' + bear.bsound + '; From: ' + bear.origin + '; Arrival Date: ' + str(
                bear.date_arival.date()) + '\n')